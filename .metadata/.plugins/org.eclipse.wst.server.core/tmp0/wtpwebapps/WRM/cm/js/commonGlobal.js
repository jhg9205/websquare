...
gcm.win.openMenu = function($p, menuNm, url, menuCode, paramObj, option) { 
	...
	if (com.util.isEmpty(url)) {
		...
		if (layout == "T") {
			...
			var options = {
				...
				closeAction : function(title) {
					if (title === "메인") {
						...
						var isOnbeforecloseall = $p.top().wdc_main.getUserData("isOnbeforeCloseAll");
						
						if (isOnbeforecloseall === false) {
							var winFrame = $p.top().wdc_main.getWindow($p.top().wdc_main.getSelectedWindowId()).$p.getFrame();
							var isClose = $p.top().scwin.closeBeforePage(winFrame);
							if (isClose === false) {
								$p.top().wdc_main.setFocus($p.top().wdc_main.getSelectedIndex());
							}
							$p.top().wdc_main.setUserData("isOnbeforeCloseAll", false);
							return isClose;
						}
			...
			var isClose = $p.top().scwin.closeBeforePage($p.top().wfm_layout.getWindow().$p.getFrame()); 
		...
		closeAction : function() {
			...
			if (popupWindow.$p.getFrameId() === null) {
				if (typeof $p.top().scwin.closeBeforePage === "function") {
					isClose = $p.top().scwin.closeBeforePage(window.$p.top().$p.getFrame());
				}
			} else {
				if (typeof $p.top().scwin.closeBeforePage === "function") {
					isClose = $p.top().scwin.closeBeforePage(popupWindow.$p.getFrame());
				}
			}
...
gcm.data.getChangeCheckedMainFrame = function($p) {
	...
	if (typeof scwin._changeCheckDcList !== "undefined") {
		return scwin;
	} else if ($w.top().$p.getFrameId() !== $p.getFrameId()) {
